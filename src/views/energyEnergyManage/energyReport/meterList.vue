<template>
	<div>
		<Row>
			...
			<MultiSelect :selectValueProp="meterIds" :optionArrProp="meterList" :widthProp="380" :maxCountProp='4'
				...
				<Form ref="addForm" :model="addForm" :rules='ruleAddForm' :label-width='15'>
					<table border class='table' width='100%'>
						...
						<tr>
							...
							<td width='80%' class='padding'>
								<FormItem prop="jlyb" class='inline' label=' '>
									<CheckboxGroup v-model="addForm.jlyb" @on-change='checkJlyb'>
						...
						<tr v-if="isadd">
							...
							<td width='80%' class="setReport">
								...
								<FormItem prop="tongjiDate" class='inline fl' label=' '>
									<DatePicker type="datetimerange" v-model="addForm.tongjiDate" :disabled='isDisabled'></DatePicker>
								</FormItem>
							</td>
						</tr>
						<tr v-if="!isadd">
						...
						<tr>
							<td width='40%' style='border:0;padding:0 10px'>
								<FormItem prop="tongjiDate" class='inline' label=' '>
									<DatePicker type="daterange" v-model="addForm.tongjiDate" :disabled='isDisabled'></DatePicker>
								</FormItem>
							</td>
							...
							<td width='33%' class='padding' style='border:0'>
								<FormItem prop="date" class='inline' label=' '>
									<DatePicker type="date" v-model="addForm.date" :disabled='isDisabled'></DatePicker>
								</FormItem>
							</td>
						</tr>
						</td>
...
<script>
	...
	export default {
		...
		methods: {
			...
			checkJlyb(data) {
				console.log(data);
				this.addForm.jlyb=data;
				this.addForm.jlybName=[];
				this.checkGroup1.map(item=>{
					this.addForm.jlyb.map(item1=>{
						if(item1==item.value){
							this.addForm.jlybName.push(item.label)
						}
					})
				})
			},
