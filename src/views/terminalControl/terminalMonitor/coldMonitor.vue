<template>
	<div style="position: relative;" class="coldMonitor">
			...
			<TabPane v-for="tab in tabs" :key="tab.key" :label="tab.label">
				...
				<Row class="statistics" v-show='isAllCount'>
					...
					<Row class="mt20">
						...
						<img :src="item.url" alt="" width="24%" class="fl mr10">
						<span class="fz18  fw700" style="display: inline-block;margin-top: 2px;">{{item.count}}</span>
						<span>h</span>
						</Col>
					</Row>
					</Col>
				...
				<Table :columns="tab.data" class="normal-table" :data="tab.coldData" no-data-text="暂无数据" style="marginTop:0" border></Table>
				<Row class="page-bottom">
					<Col span="12" class="page-text">当前显示<span class="pageColor">{{tab.coldData.length}}</span>条记录,共<span class="pageColor">{{dataCount}}</span>条记录</Col>
					<Col span="8" :offset="4">
...
<script>
	import highSpeedImg from '@/assets/images/terminalControl/highSpeed.png'
	import lowSpeedImg from '@/assets/images/terminalControl/lowSpeed.png'
	import MediumSpeedImg from '@/assets/images/terminalControl/MediumSpeed.png'
	...
	export default {
		...
		data() {
			return {
				...
				legend: ['用冷量', '环比用冷量', '2018年同期用冷量', '平均温度'],
		...
		mounted() {
			this.tabs = [{
					label: "总用量监测",
					data: this.columns1,
					coldData: this.tableData1,
				},
				{
					label: "按建筑(区域)监测",
					data: this.columns2,
					coldData: this.tableData2,
				},
				{
					label: "按板换侧监测",
					data: this.columns3,
					coldData: this.tableData3,
				},
				{
					label: "按用户监测",
					data: this.columns4,
					coldData: this.tableData4,
				}
			];
